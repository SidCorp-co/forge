...
- **A box whose masters were left unreachable now puts itself right.** Each is restarted where work
  is waiting, and whatever is left standing is named once for the whole box. A restart the box
  tried and could not make is reported as one it could not make, rather than as done.
