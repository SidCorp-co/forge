...
## [Unreleased]
- Platform admins now have an operations alerts view covering the five conditions that quietly stall the pipeline — orphaned jobs, stuck jobs, runner starvation, spend spikes and failing automation — and get a notification when one crosses into warning or critical, which clears itself once the condition does.
...
- Pipeline agents now receive a lean index from `forge_step_start` on complex issues, reducing token usage per step on issues with large plans or descriptions.
