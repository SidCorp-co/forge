...
- **On the issues page you can now reach every issue — including closed and draft ones. The default view shows all active and closed work, and a one-click control (Select all on the classic app, the new Drafts tab on the v2 app) reveals draft proposals.**
  *Technical: Discoverable draft/closed issues on both web surfaces. v1 (packages/web status-multi-select.tsx): sticky Select all / Clear header inside the dropdown (Select all ticks all statuses incl. draft, which use-issues-page.ts maps to a draft-inclusive query, dropping the statusNot:['draft'] guard) + honest trigger label ("Active (no drafts)" default, "All statuses" when full). v2 (packages/web-v2 derive.ts/issues-screen.tsx): the "All" tab now sends statusNot:['draft'] only (closed/released shown) + a new "Drafts" tab (status:['draft']). ISS-236 default (drafts hidden until opted in) preserved. Merges 7a4300a (v1) + 88dca6a (v2).*
