...
- **Why a release will not start now names the box, its state and the switch that changes it.** It
  ...
  so.
- **A held release names each runner by what its Runners tab calls it, not a second, staler
  name.** Three refusals also answer with the same sentence release readiness already gives,
  rather than a separate wording of their own.
