...
- **A box that could not reach a repository's remote said your work might be lost, and it was wrong.** The check signs in as the repository is configured to push, and a removed worktree says so in the log.
...
- **A runner box takes new work once a job's agent has finished.** A pane whose agent had stopped,
  or was waiting on a question nobody answers, held its slot until a person noticed, refusing every
  project on the box.
