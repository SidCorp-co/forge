...
describe('seedBuiltinSkills', () => {
  ...
  it('updates on content change and bumps version + content_hash', async () => {
    ...
    const second = await seedBuiltinSkills(harness.db as never, { skillsRoot });
    expect(second).toEqual({ inserted: 0, updated: 1, unchanged: 1 });
  ...
  it('seeds all 8 bundled forge-* skills from the real package directory', async () => {
    // Use the real `forge/core/skills/` root — ensures the shipped files parse
    // and the seeder works end-to-end without the tmp-dir fixture.
