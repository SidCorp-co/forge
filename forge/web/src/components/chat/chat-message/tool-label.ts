...
function formatMcpLabel(name: string, input: Record<string, unknown>): string {
  ...
  switch (toolName) {
    case 'forge_coolify_deploy': {
      const actionMap: Record<string, string> = {
        list: 'Resources', deploy: 'Deploy', status: 'Status', get: 'App',
        logs: 'Logs', start: 'Start', stop: 'Stop', restart: 'Restart',
        envs: 'Envs', 'set-env': 'Set env', 'delete-env': 'Delete env', 'cancel-deploy': 'Cancel',
      };
      const label = actionMap[action] ?? 'Coolify';
      return `${label}${id ? `(${id.slice(0, 8)})` : ''}`;
    }
    case 'forge_issues': {
