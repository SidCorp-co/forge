...
export function loadManifest(root) {
  ...
  const unknown = Object.keys(raw).filter((k) => !k.startsWith('$') && !MANIFEST_KEYS.has(k));
  if (unknown.length) {
    fail(`unknown ${MANIFEST_NAME} key(s): ${unknown.join(', ')} — supported: ${[...MANIFEST_KEYS].join(', ')}`);
  }
  ...
  switch (c.type) {
    ...
    case 'absence': {
      if (typeof c.from !== 'string' || typeof c.to !== 'string') {
        fail(`${where}: absence needs "from" and "to"`);
      }
      if (c.from === '*' || c.to === '*') fail(`${where}: absence does not take "*"`);
      known(c.from);
      known(c.to);
      if (c.from === c.to) fail(`${where}: absence needs two different modules`);
      break;
    }
    default:
      break;
