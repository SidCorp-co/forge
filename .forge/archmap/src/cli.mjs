...
import { ContractError, evaluate, REMEDY } from './contracts/index.mjs';
import {
...
import { collectGraph, loadBaseline, buildReport, PROVIDERS, VENDOR_GLOB, baselineKey, baselineValue } from './check.mjs';
...
const USAGE = `archmap — architecture conformance from a declared manifest
  ...
  archmap lock <id>         freeze one contract's current violations into
                            .arch.baseline.json, so locked means no NEW ones.
                            Writes the snapshot only — setting status: locked
                            in .arch.json stays your edit, to be reviewed.
...
export function main(argv) {
  ...
  const scoped = buildScope(root, cmd === 'lock' ? [] : paths);
  const { manifest, graph, failures, notes } = scoped;
  if (cmd === 'graph') return doGraph(graph, flags, scoped);
  if (cmd === 'lock') return doLock(root, manifest, graph, paths);
...
/**
 * Snapshot one contract's current violations into `.arch.baseline.json` (SPEC §8.2).
 *
...
function doLock(root, manifest, graph, paths) {
  const id = paths[0];
  if (!id) die('archmap lock needs a contract id — `archmap lock <id>`');
  const contract = manifest.contracts.find((c) => c.id === id);
  if (!contract) {
    die(`no contract "${id}" in .arch.json — declared: ${manifest.contracts.map((c) => c.id).join(', ')}`);
  }

  let findings;
  try {
    findings = evaluate(manifest, graph).findings.filter((f) => f.contract === id);
  } catch (e) {
    if (e instanceof ContractError) die(e.message);
    throw e;
  }
  ...
  for (const [k, v] of existing.entries) {
    if (!k.startsWith(`${id}|`)) frozen[k] = v;
  }
  for (const f of findings) frozen[baselineKey(f)] = baselineValue(f);

  const doc = {
    version: 1,
    frozen: Object.fromEntries(Object.keys(frozen).sort().map((k) => [k, frozen[k]])),
  };
  writeFileSync(join(root, BASELINE_NAME), `${JSON.stringify(doc, null, 2)}\n`);

  const mine = findings.length;
  const others = Object.keys(frozen).length - mine;
  process.stdout.write(`archmap lock: froze ${mine} violation(s) of "${id}" in ${BASELINE_NAME}`);
  process.stdout.write(others > 0 ? `, alongside ${others} already there\n` : '\n');
  if (contract.status !== 'locked') {
    process.stdout.write(`\nThe snapshot alone gates nothing. Set "status": "locked" on "${id}" in\n`);
    process.stdout.write('.arch.json and commit both — the manifest is a governance file, so that\n');
    process.stdout.write('edit is yours to make and to have reviewed, not this command\'s.\n');
  }
  return E_OK;
}
