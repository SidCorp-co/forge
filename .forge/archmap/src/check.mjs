...
export function loadBaseline(root) {
  ...
  if (!existsSync(path)) return { entries: new Map(), present: false };
  try {
    const raw = JSON.parse(readFileSync(path, 'utf8'));
    const frozen = raw.frozen ?? [];
    const entries = Array.isArray(frozen)
      ? new Map(frozen.map((k) => [k, null]))
      : new Map(Object.entries(frozen));
    return { entries, present: true };
  } catch {
    return { entries: new Map(), present: false, unreadable: true };
  }
}
...
export function baselineKey(f) {
  return f.type === 'fan-out' ? `${f.contract}|${f.from}` : `${f.contract}|${f.edge}`;
}

/** What a freeze records for this finding: a number that may not rise, or null for "present". */
export function baselineValue(f) {
...
function isFrozen(baseline, f) {
  const key = baselineKey(f);
  if (!baseline.entries.has(key)) return false;
  const was = baseline.entries.get(key);
  if (was === null || was === undefined) return true;
  const now = baselineValue(f);
