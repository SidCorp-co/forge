...
| Piece | One line |
|---|---|
| Pipeline engine | 15-status, per-stage auto-run or human-gate; shorten/extend/replace per project (single pre-prod gate = `tested`) |
| Device runners | paired devices spawn `claude` locally, stream events; headless Rust daemon (`packages/runner`) |
