...
| C | plan-gate dispatch backstop | `packages/core/src/pipeline/orchestrator.ts` (`considerEnqueue`) + `packages/core/src/pipeline/plan-gate-guard.ts` | Catches issues already sitting at `approved`/`plan:null` from before the writer guard existed — routes to `clarified` (or `needs_info` if a `plan` job already ran and is still blank) and posts an operator comment. Also routes a `reopen` entered directly from `needs_info` back to `needs_info` instead of dispatching an unscoped `fix`. |
| A | `hasHumanAnswerSince` | `packages/core/src/pipeline/bounce-replay-guard.ts` | `needs_info` only releases on a comment with `isAi=false AND author_device_id IS NULL` — an agent's own comment (including an MCP/PAT comment, `isAi=true` with no device) can never count as the human answer it claims to be. |
...
All four share the same two invariants:
