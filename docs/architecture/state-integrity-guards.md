...
| A | ~~`hasHumanAnswerSince`~~ | **deleted — RFC 0002 INV-6** | Was: `needs_info` only released on a comment with `isAi=false AND author_device_id IS NULL`, so an agent's own comment could never count as the human answer it claimed to be. It went with `bounce-replay-guard.ts`. **Known cost: nothing now stops an agent from leaving `needs_info` on its own note.** RFC 0002 accepts that trade — the same release condition refused four legitimate resume attempts on ISS-163 — but this row is the record that the anti-fabrication half was real, not just an actor gate. |
