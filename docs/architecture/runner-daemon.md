...
- Can create a git worktree under `<repo>/.worktrees/<branch>` when core hands a `worktreeBranch` ([`workspace/worktree.rs`](../../packages/runner/crates/forge-runner-core/src/workspace/worktree.rs)) — **but core never sends that field**, so in practice every job runs in the repo root and the agent cuts its own checkout under `.claude/worktrees/<slug>`. Measured on dev1 2026-08-26: `.worktrees/` did not exist; six agent worktrees did. Anything reasoning about "the job's checkout" must FIND it (`git worktree list --porcelain`), not derive a path — see [`workspace/salvage.rs`](../../packages/runner/crates/forge-runner-core/src/workspace/salvage.rs).
...
- **Self-update** ([`update/mod.rs`](../../packages/runner/crates/forge-runner-core/src/update/mod.rs)): daemon checks the manifest ~30s after start then every 6h. With `update.auto` set it downloads the asset for its target triple, verifies sha256, atomically replaces the running executable, drains in-flight work to idle, then **`exit(0)` so systemd `Restart=always` relaunches the unit** and re-execs the new binary. `forge-runner update` does the same on demand.
  - ⚠️ **Restart MUST be name-agnostic (`exit(0)`), never a hardcoded `systemctl restart forge-runner`.** A hardcoded unit name only bounces the default unit; any instance under a different unit name (multi-account `forge-runner-<id>` units — e.g. `forge-runner-ai005`, `forge-runner-qa`) would download the new binary to the shared `~/.local/bin/forge-runner` but never re-exec it, staying on the stale in-memory build and re-applying the same update every 6h forever. Bug shipped in ≤0.6.0, fixed in 0.6.1 ([`runner-v0.6.1`](https://github.com/SidCorp-co/forge/commit/52fa313d)); the credential-watch path was always correct and is the reference. **Escape from a stuck daemon needs ONE manual restart onto a fixed (≥0.6.1) binary** — the fix can't self-apply because the buggy restart code is what's running. Same invariant binds any future relaunch trigger (cred rotation, config reload).
