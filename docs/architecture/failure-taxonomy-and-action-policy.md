...
| 757/630 | `finalize-failure.ts` stamps the exhausted runner's `rateLimitedUntil` **before** the retry decision reads it (a genuine ordering bug found in review round 1 — the old fire-and-forget stamp ran too late for `all_devices_exhausted` to ever fire). `retry.ts` distinguishes "every online box is limited" (park + notify) from "every box is merely offline" (wait quietly) via `onlineCapableDeviceIds`'s health-gated vs. unfiltered sets. | `jobs/finalize-failure.ts`, `jobs/retry.ts`, `runners/select.ts` |
