...
| AC1/AC3 (the write itself) | `failureStamp(kind, reason, action?)` in `pipeline/failure-classifier.ts` is the only way to write the four failure columns, so a terminal write cannot persist a kind while omitting the action or freezing the version. Five writes still hand-wrote them: the runner/job-type gate picked `code` with no action, no version and no `finishedAt` (17 rows, kinetrak 2026-08-20 — `pipeline_run_step_durations` never saw them), and four more froze `classifierVersion: 3` while the taxonomy was at 8 (32 rows through 2026-08-21). The gate now persists `classifyFailure`'s own verdict for a string the classifier already owns (`runner_unsupported_type` is a `PERMANENT_PATTERNS` entry), and the other four derive the action from the kind — which is exactly what `deriveActionFromKind` did for a NULL column, so no behaviour moves; the row simply stops lying about which taxonomy judged it. | `jobs/dispatcher.ts`, `jobs/loop-monitor.ts`, `jobs/handle-resume-failed.ts`, `pipeline/failure-classifier.ts` |
...
`packages/core/tests/integration/failure-stamp-call-sites-e2e.test.ts` covers the write itself: the runner/job-type gate records the classifier's verdict with `finishedAt` set, and the resume-abort tail stamps the version in force. `packages/core/src/pipeline/failure-stamp-wiring.test.ts` scans the source so a numeric `classifierVersion:` literal cannot come back — nothing in the type system forbids one.
