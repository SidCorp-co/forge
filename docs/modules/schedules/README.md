...
- A schedule is exactly one of two kinds, set at creation and immutable-by-convention thereafter (switching kind mid-life is allowed by the API but the plan/UX assumes a schedule "is" one kind): [`kind: 'prompt'`](#2-kind-prompt) or [`kind: 'script'`](#3-kind-script-no-agent-no-llm).
