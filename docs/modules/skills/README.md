...
- **Removed — do NOT reintroduce:** the meta disk-install path (`syncManagedSkills` / `resolveInstallableSkills`) — channels 2+3 replaced it; the override/fork mechanism (`projectSkillOverrides`, ISS-388) — shadow-by-name only; the per-project `install_only` bridge (`SHARED_INSTALL_ONLY_SKILLS`, ISS-742) — deleted 2026-08-10; the plugin channel owns meta, and Forge-owned governing agents (`forge-reconcile`, `forge-verify-skill`) are inlined into their job prompt from the global row and reserved in `META_SKILL_NAMES` so no project can adopt them.
