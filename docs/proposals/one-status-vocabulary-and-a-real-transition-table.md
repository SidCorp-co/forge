...
6. **Sweep the teaching material** — system-prompt generation, UI pickers, `docs/modules/
   ...
   `plugin/skills/issue-flow/SKILL.md`, which the invariant on `AUTONOMOUS_DRIVER_STATUSES` names
...
| The cross-repo half ships on `forge-plugin`'s clock. Between the two deploys, the skill's status table and the kernel's disagree — the exact shape that produced 4,806 wrong calls when the drive prompt and the guide diverged (`run_session.rs` invariant) | both repos, for the length of the gap |
...
| History keeps the old spelling forever: `comments.stage` (13 rows), `activity_log.payload` (4,488) and 7 `pipeline_runs.metadata.gateStatus` rows still say `released`, because a record says what a thing was called when it happened. Four SQL readers carry both spellings and a invariant saying why; drop either and a metric silently loses one side of 2026-09-10 | every reader of a cross-boundary metric |
