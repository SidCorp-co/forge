...
| [fact-applicability.md](fact-applicability.md) | Proposed, nothing implemented. A `projectFact` declares its own scope in prose and nothing reads it: `ux-contract` is 4,116 chars at `alwaysInject`, opens "BINDING for any issue that adds/changes UI in `packages/web-v2/`", and 9 of the last 40 commits touch that path — so ~78% of prompts carry it for nothing. The mirror case cost a real answer the same day: ISS-919 cut `runner-v0.12.0` only because its own comment said so, and no skill or fact covers release artifacts. **Records one design killed by reading the tree**: core cannot evaluate a `pathsChanged` condition at dispatch, because on a `drive` pass the branch does not exist yet — so the condition ships as data in the on-demand index and the agent, which holds the diff, evaluates it. Replaces the `alwaysInject` and `required` booleans with one `applies` axis; adds an atom gate measured at 31/31 green today | 2026-09-06 | a decision on the one open question — whether a triggered fetch actually gets fetched, which `alwaysInject` (ISS-521) exists because it did not |
