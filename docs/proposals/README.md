...
| [pat-comments-un-park-the-driver.md](pat-comments-un-park-the-driver.md) | Open decision, no code proposed — `answer-resume.ts` resumes a `needs_info` issue on any `user`-actor comment, and its `cm:guard` rests on "every AI comment path emits a `device` actor", which is false for `POST /api/issues/:id/comments` under a PAT — the transport the autonomous driver prompt mandates. Measured 2026-09-06 on ISS-593: the driver parked at `23:49:24Z`, posted one more comment at `23:51:18Z`, and the issue was back at `open` one second later, logged as `human answered`. So a driver that parks and then writes anything un-parks itself, and the reconciler re-dispatches within the minute | 2026-09-06 | a human picks between stamping agency on PAT writes, refusing a resume from the principal that wrote the park, and making a park declare what resumes it |
