...
| [conversation-turn-runner-is-not-transport-neutral.md](conversation-turn-runner-is-not-transport-neutral.md) | Step 1 of five landed on ISS-1002 (the turn runner, the screening decision, and the store held unreachable from every adapter tree); steps 2 to 5 are not implemented. The collector window still lives in a process, the proactivity guards do not exist, the @-mention gate that currently bounds cost has not been replaced, and the Forge UI is not yet the second adapter | 2026-09-14 | the collector window as rows, the guards, the gate removed by the change that replaces it, and the second adapter that proves the four functions |
