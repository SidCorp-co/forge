...
| [retrieval-v3-rerank-chunks.md](retrieval-v3-rerank-chunks.md) | Proposed, no code written — the retrieval techniques the predecessor (`forge-agents`) ran that `memory/search-service.ts` does not: a reranker behind `hybrid`, a chunked memory model as a sibling `memory_chunks` table so the natural key and every consumer of one-row-per-key stay untouched, one-hop issue-relation expansion, and an identifier-aware tsvector gated on measured keyword contribution. All per project, on three `app_config` columns, with a resumable reindex job on the flip and a UNION read that is correct mid-migration. Read against the tree 2026-09-04; filed as drafts ISS-904 (phase 0), ISS-905 (1+3), ISS-906 (2), ISS-907 (4, opened only on evidence), all blocked on ISS-904 | 2026-09-04 | open ISS-904; the edges release the rest |
