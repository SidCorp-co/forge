...
| [conversation-turn-runner-is-not-transport-neutral.md](conversation-turn-runner-is-not-transport-neutral.md) | Four of five steps landed: the turn runner (ISS-1002), then the collector window as rows, the three derived proactivity guards and the deletion of the @-mention gate (ISS-1004). **Step 5 is the one left**: `/api/conversations` lists, reads, renames and deletes rooms but has no send, no `web` transport is registered, and every conversation surface in web-v2 still reads `agent_sessions` — so two paths are live, the one people use and the one the store has. Records why it was left whole rather than half-landed: `ChatScreen` carries model pick, fork, rerun, per-turn edit and regenerate, none of which `conversation_messages` can represent, so the port is a product decision about which of those are conversation and which are run telemetry | 2026-09-14 | that product decision, then the four `web` functions and one send endpoint, with the `agent_sessions` chat reads deleted in the same diff |
