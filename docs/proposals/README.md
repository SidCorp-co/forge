...
| [the-forge-ui-conversation-cannot-reach-a-runner.md](the-forge-ui-conversation-cannot-reach-a-runner.md) | ISS-1004 step 5 landed the Forge UI as the second conversation adapter and **this is what the port cost**: the chat surface was a runner-hosted Claude Code session with the repository checked out, and is now an in-core conversation turn with a read-only project toolset and no working tree. Six affordances went with the runner — model pick, runner pick, fork, rerun, per-turn edit, regenerate — each of which names a RUN and each of which is still on the session surface. Names the seam that reverses it: `divertBeforeTurn` on the turn inputs, the same one `rocketchat/turn-inputs.ts` uses for the `agent` answer mode | 2026-09-14 | a transport-neutral extraction of `agent-chat.ts` and its completion bridge, a 202-and-a-push send route, and a product answer on whether the Forge UI's agent is a project assistant or a hand on the repository |
