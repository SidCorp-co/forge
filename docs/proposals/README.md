...
| [browser-session-survival.md](browser-session-survival.md) | Open residual — a UI stage's browser crashes mid-session and the recovery is paid by hand, which is anhome's 3.38× clarify p95 | 2026-08-26 | measure the crash rate by cause, then choose between a session-surviving browser and a per-attempt isolated one |
