...
| [agent-driven-pipeline.md](agent-driven-pipeline.md) | Draft (owner session 2026-08-19) — not implemented; upgrade to an RFC once mode switch + status vocabulary are agreed | Phase 0 (embed skills in runner) → 2 (phase journal) ship alone; 3–5 are the mode itself |
| [release-gate-and-deploy.md](release-gate-and-deploy.md) | Draft (owner session 2026-08-24) — design agreed, checklist anchored to the tree, not implemented; completes the autonomous mode's terminal half | 5 waves, gated per project: kernel truths → the gate → release path → deploy+proof → schedule+UI |
