...
| [a-turn-row-goes-stale-when-the-transcript-grows-past-it.md](a-turn-row-goes-stale-when-the-transcript-grows-past-it.md) | Open residual — `syncTurnsWithMessages` updates a changed entry only when the array length is unchanged, so a derive that appends AND rewrites its tail in the same pass leaves `agent_session_turns` holding an earlier state of that turn; measured against a real Postgres, both the full re-derive and the incremental one land there, and the same events derived in one pass do not | 2026-09-15 | the growth branch walks back over the entries it is not appending, plus a decision about the rows already on disk |
...
| [retrieval-v3-rerank-chunks.md](retrieval-v3-rerank-chunks.md) | Phases 0, 1, 2 (backend) and 3 shipped (ISS-904, ISS-905, ISS-906): hybrid search records what each list gave, an admin breakdown reads it, four `app_config` flags exist; with them on, an agent's hybrid search is reranked by the fast model (one-in-five holdout as control) and issue hits pull in their one-hop `blocks`/`relates` neighbours; a project flipped to `chunked` through the memory-model operation embeds ~1,200-character passages into a sibling `memory_chunks` table, joined on generation, searched UNION the not-yet-chunked flat rows, with a resumable reindex behind five states. The Project Settings Memory tab draws those states (ISS-908). Still proposed: phase 4, an identifier-aware tsvector gated on measured keyword contribution (ISS-907, opened only on phase-0 evidence) | 2026-09-05 | ISS-907 once a week of forge-dev breakdown rows exists; retire once it is decided |
| [fact-applicability.md](fact-applicability.md) | Proposed, nothing implemented. A knowledge entry declares its own scope in prose and nothing reads it. The mirror case cost a real answer: ISS-919 cut `runner-v0.12.0` only because its own comment said so, and no skill or entry covers release artifacts. **Records one design killed by reading the tree**: core cannot evaluate a `pathsChanged` condition at dispatch, because on a `drive` pass the branch does not exist yet — so the condition ships as data in the on-demand index and the agent, which holds the diff, evaluates it. Replaces the three-valued `injection` column with one `applies` axis; adds an atom gate measured at 31/31 green today. Its original worked example, `ux-contract` at 4,116 chars and `injection: always`, was settled bluntly instead: ISS-1068 retired the subsystem and moved all 13 of those entries to `on_demand` | 2026-09-17 | a decision on the one open question — whether a triggered fetch actually gets fetched, which ISS-1068 now makes observable in the field on 13 projects |
| [master-drives-subagents.md](master-drives-subagents.md) | Decided, nothing implemented — the owner ruled 2026-09-09 that a master assigns work as an in-session subagent (`Task()` + `isolation: "worktree"`), a child never outlives its parent, and a session limit is retried on a 5-minute cadence. Records two findings that killed the obvious implementation: the 5-minute cadence **already exists** at every link except one (`next_poll_delay` clamps to `[30s, 300s]`; a limited fleet already defers without charging budget, owner call 2026-08-12), and capping `rateLimitedUntil` to 5 min is strictly worse than today because a hold carries one auto-release per lineage (`jobs/hold.ts:52`). The real gap is a silence: `detect_usage_limit` has exactly one caller, so a master/run session that hits its cap reports nothing. Carries the drawn figure and the `docs/flows/` worklist the shipping PR inherits | 2026-09-10 | stage 1 (the master's limit report reaching core) needs no release; stages 2-3 each cost a fleet restart, so they batch |
| [workspace-drilldowns.md](workspace-drilldowns.md) | Open residual, no code proposed — five figures on the workspace dashboard (ISS-988) count date-windowed aggregates no client-reachable route can list: a heartbeat day, a flow week, a run-failure lane, a session-failure reason, a job-type flow node. They ship drawn without a door rather than linked to approximately the right rows. Closing it needs four org-scoped record lists that take the window the figure was drawn over | 2026-09-12 | the same pass as the per-project dashboard, which ISS-988 puts out of its own scope |
| [fg-type-classes-swallow-every-tailwind-utility.md](fg-type-classes-swallow-every-tailwind-utility.md) | Open decision, no code proposed — the type scale in `styles/tokens.css` is unlayered and Tailwind v4 utilities live in `@layer utilities`, so a colour or weight utility on the same element as any `fg-*` class does nothing. Measured on beta at `d0389485c` closing ISS-1033: the stale mark on the Issues list's Waiting column computed `rgb(118,125,138)` at weight 500, identical to a fresh row, although `--amber-600` was defined and the utility was generated. Nine call sites across four screens and three error messages carry the same dead classes. ISS-1033 paid the local price (`.fg-caption-stale`, same specificity, later source order) rather than restyling surfaces it does not own | 2026-09-17 | a human decides, per call site, whether the intended style is the wanted one; then the scale moves into a cascade layer and something asserts a computed style in a real browser, which no check here can do |
| [an-mcp-list-title-is-char-stripped-and-not-framed.md](an-mcp-list-title-is-char-stripped-and-not-framed.md) | Open residual, measured, no code proposed — `mcp/tools/forge-issues.ts:serializeListRow` char-strips an issue title and does not frame it, while `prompt/user.ts` and `serialize` both `markUntrusted` the title and the description. The gap is priced in the projection's own `cm:why` (a ~120-char DATA banner per row against the token cap the lean projection exists for, ISS-428/ISS-532) and was found by ISS-1085 slice 3, which needed to say in a criterion exactly where Sentry text is framed and could not say "everywhere". `integrations/sentry/chokepoint.test.ts` asserts the current behaviour, so a change to it is a red test rather than a silent re-decision | 2026-09-17 | a measured 50-row `forge_issues list` against the MCP token cap, then a human picks between framing it, a bounded second frame vocabulary, or saying so in the tool description |
| [a-bounded-sentry-pull-does-not-resume.md](a-bounded-sentry-pull-does-not-resume.md) | Open residual, bounded and announced in code, no fix proposed — ISS-1085 slice 3's scheduled Sentry pull walks the provider's cursor to a declared bound of 1,000 issues per target per tick and starts again at page one on the next. Sentry orders by last seen, so the tail past the bound is the same tail every tick. A persisted cursor was considered and NOT taken, and the file records why: the list reorders between ticks, so resuming from an old cursor trades "cannot see the quiet tail" for "cannot see part of the busy head", which is a worse silence and a harder one to notice. Three real options are priced. Reaching the bound is announced in the run record and in the delivery row rather than passed off as a complete listing | 2026-09-17 | one measurement nobody here has — how many unresolved error-level issues a real target holds — then a human picks between ordering by first seen, dropping the bound, or narrowing the query |
| [a-closed-forge-issue-does-not-tell-sentry.md](a-closed-forge-issue-does-not-tell-sentry.md) | Open residual, capability shipped and untriggered — ISS-1085's contract runs both ways and only the inbound half is wired. Slice 2 landed `sentry.issue.set-status` and all four Sentry status values; slices 3 and 4 landed the pull and the webhook. Nothing in `issues/`, `pipeline/` or `release-batch/` enqueues the outbound call, so three rows of that contract (close → `resolvedInNextRelease`, release verified → `resolved`, dropped → `ignored`) have a method and no caller, and from Sentry's side every issue Forge ever closed is still unresolved. No slice of the four owns the wiring — it is a gap in the decomposition, and it is why ISS-1085 does not close with slice 4. Three decisions are priced in the file, the sharpest being that a trigger keyed on `mergedCommitSha` fires for about one close in nine | 2026-09-17 | a human answers which close counts, whether the `resolved` row waits for a release-side owner, and what a refused dispatch does to the transition that enqueued it |
...
|---|---|---|
| `master-orchestration.html` | Consumed by ISS-917 + ISS-933 (2026-09-08). Its step 8 table is done end to end: the daemon holds the socket and the ledger, the box claims from `/api/devices/me/pool`, a run session opens from the box and closes on three observed marks, and step 7's deletions landed — push, the dispatch tick, `maxConcurrentIssues` (migration 0206) and the supervision cluster are out of the tree. The §10 residuals that outlived it: what a chat session's run means is ISS-964, and subagent-to-subagent `job_events` reads stay refused by having no endpoint | git history · `docs/flows/lifecycle-pipeline.html` + `lifecycle-pipeline.html` · `packages/runner/crates/forge-runner-core/src/runner/` |
...
| `rocketchat-bot.md` | Lane A shipped + live; its own body carried three SHIPPED markers (ISS-609, ISS-671/672/674/675/687/727) while this table said otherwise | `packages/core/src/assistant/providers/` |
| `chat-provider-standardization.md` | P1 shipped 2026-07-02 in `934dab4a`, the commit naming its own ISS-604; P2 2026-07-03 (ISS-609); P3 write tools live in `assistant/tools/` | `packages/core/src/assistant/providers/` |
