...
| [cli-data-surface.html](cli-data-surface.html) | Drawn architecture, nothing built — an agent-facing CLI over REST, and the collapse of the two half-overlapping issue implementations it depends on. Measured 2026-08-30: `mcp/tools/forge-issues.ts` (1,359 lines, reaches drizzle directly) and `issues/routes.ts` (783) share 4 modules and carry ~6 each the other lacks; MCP builds its own `create` instead of `issues/creator.ts`; only MCP can set a `blocks` edge | 2026-08-30 | raise REST to MCP's capability first, then MCP tools become thin adapters and the CLI is a third caller of one surface — never a third implementation |
