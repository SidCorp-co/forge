...
| [conversation-turn-runner-is-not-transport-neutral.md](conversation-turn-runner-is-not-transport-neutral.md) | Not implemented. ISS-1001 (merged `3e8faa1a`) landed the conversation store, the schema, migration 0241 and the four adapter ports in `conversations/ports.ts`, and `transport-free.test.ts` holds the store to its side in both directions. **The turn runner did not land**: Rocket.Chat's `connection-manager.ts` still calls `openConversation` and `recordDelivery` directly, and nothing consumes `deliver` or `fetchHistory` at runtime, so a second adapter is four functions plus a copy of the turn path. ISS-1001's criterion 35 is recorded on that issue as a FAIL for exactly this, rather than reworded | 2026-09-14 | a transport-neutral turn runner; two questions named in the file (where the screening decision sits, entangled with ISS-997; and whether the collector window becomes rows) |
