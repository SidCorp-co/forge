...
| **1** | **done** — `1ab9162f`, `06480a45`. duplex spawn + inverted completion task, **chat only**: `Stdio::piped()`, `--input-format stream-json`, `--replay-user-messages`, `--session-id`; prompt moves off `-p` into a `work` message; `send` stops returning `NotImplemented` (`claude_code.rs`). **Plus what A3 was:** the session-scoped permit, the chat idle window that closes an abandoned session, and `--resume` rehydrate on the next turn — the same change, because this phase creates the first object that outlives a turn and therefore the first release event a permit can wait on | chat's per-turn one-shot lane — `chat.rs`'s module header, `resume_id` on `Turn` / `handle_start` / `handle_send` / `run_turn`, and the two `cm:guard`s in `handle_send` and `run_turn` that exist **only** because a follow-up respawns. **Neither rule dies with its guard.** The model picker must still be honoured per turn — measured 2026-08-29, an in-band `/model sonnet` on a live stream-json session emits a fresh `system:init` and the next turn's `modelUsage` carries the new model, so the picker becomes a `/model` message and needs no respawn lane. The refresh rule now has one, and the earlier reading of it was wrong: `refresh::refresh` is called from `run_turn`, which is entered **once per turn**, not once per spawn — the two coincide today only because a turn *is* a spawn. Phase 1 changes what happens inside `run_turn`, not how often it is entered, so refresh stays exactly where its `cm:guard` puts it and keeps running every turn. What is new is that the fast-forward now runs against a checkout a LIVE process holds, so a refresh that moved the checkout must be announced to the resident session in-band — under one-shot that was free, because the process was always newer than the refresh | chat end-to-end; no pipeline job touched |
| **2** | **done** — `1d886039`. `TurnEnded` + `StateChanged` events · core classifies turn end · loop-monitor quiet reads `runtimeState` | — | verify + integration |
| **3** | **done** — `bc0fbee5`, `36115b9e`, `948dfef3`; carried to the fleet by runner 0.9.0 (`0a5a7669`). pipeline jobs on duplex · `answer` + ack-is-commit in `answer-resume.ts` · durable core-side residency deadline | the 25 s synthetic `progress` beat (`daemon/dispatch.rs#consume`) — a session that declares its state does not need a beat asserting one | **met** — 3 projects on duplex as of 2026-08-31: forge-dev, epodsystem-core, getcontent, every bound runner on 0.9.2 |
| **4** | **partly done.** The runner arms all five kinds (`daemon/inbox.rs`) and core stamps and audits them, but a kind is only reachable once something CALLS `requestSessionSend` with it — and on 2026-08-31 exactly two things did. `inject` got its caller in `871c0b09` (ISS-888 item 2: `POST /api/issues/:id/steer` + `forge_steer`); `answer` has had one since phase 3. **`work`, `checkpoint` and `cancel` still have none**, so they are built and unreachable. `inject` · `checkpoint` · `cancel` folded into the `job.cancel` key space | `abort`'s signal path (`claude_code.rs`) → becomes checkpoint-then-close | verify + live |
| **5** | default flips to duplex; `print` reachable only by explicit opt-out, one release. Not taken: the default is still `print` (`runners/adapters/claude-code.ts`) while the sample is 3 of 30 active projects | — | fleet-wide, measured — sample opened 2026-08-31, nothing to compare yet |
