...
- **Status:** Accepted
- **Date:** 2026-04-24
- **Context:** [RFC 0002](../rfcs/0002-replace-strapi-with-hono-drizzle.md), [ADR 0010](0010-clean-break-from-strapi.md), Phase 2.6 `forge/web` rewire onto `forge/core`.
- **Supersedes:** none.

## Context

Phase 2.6 rewires `forge/web` off Strapi onto `forge/core`. The previous client hit Strapi's `/api/*` with a bearer token in `Authorization` and unwrapped Strapi's `{ data, meta }` envelope.
...
3. Keeps the web bundle decoupled from `forge/core/src` — `core` is a server app, not a library.
...
| Option | Verdict |
|---|---|
| **OpenAPI generator** (`@hono/zod-openapi` → `openapi-typescript`) | Rejected. `forge/core` uses plain Hono + `zValidator`. Adopting OpenAPI requires rewriting every route to the OpenAPI variant — scope creep. Revisit post-v0.x if we publish an external API. |
| **tRPC / Hono RPC client** | Rejected. Couples `web` to a TS-only contract; the REST surface must stay usable by `forge/dev`, MCP consumers, and third parties. RFC 0002 targets REST. |
...
| **Manual TS types maintained in `forge/web`** | Rejected. Drifts from the DB; the exact failure mode we are avoiding. |
...
`forge/web` talks to `forge/core` through a hand-written `apiClient` in `forge/web/src/lib/api/client.ts`.
...
3. **Types.** `@forge/contracts` — a workspace package under `forge/contracts/` — re-exports:
...
- **`forge/core` must keep its public surface stable.** Renaming a table column is now a cross-package change. Acceptable for an internal monorepo; we accept the coordination cost for single-source-of-truth.
- **`forge/contracts` exports a minimal surface.** Only what clients need (rows + request inputs + response wrappers). Internal helpers stay in `forge/core`.
...
- **MCP typings.** MCP uses device tokens; its types belong in `forge/dev`, not `@forge/contracts`.
