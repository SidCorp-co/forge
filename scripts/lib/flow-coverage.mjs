...
// a cm:flow annotation sits on a comment line ABOVE the code it names, so the step's line falls just outside its own function — hence the tightest containing function, or failing that one declared within 5 lines below
const LOOKAHEAD = 5;
...
// the statement a step NAMES is the nearest one at or below the annotation, never the tightest enclosing one — a `cm:flow` above a function sits inside no statement, and widening to enclosing statements would answer with the whole function body and make this level identical to the function level it exists to distinguish
