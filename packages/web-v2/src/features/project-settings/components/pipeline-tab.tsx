...
export function PipelineTab({
	...
	// cm:guard the working copy is the FULL fetched config, not the slice this screen edits, because the PATCH merge is shallow: a save built from named fields alone drops every key this build does not know about, on a document other screens and MCP also write.
	...
	if (cfgQ.isError) {
		...
		return (
			<Card>
				<CardContent>
					<ErrorState
						message={formatApiError(cfgQ.error)}
	...
	const dirty =
		...
		entryGateOpen(server) !== entryGateOpen(draft);
		...
		<Card>
			<CardContent>
				...
				<p className="fg-body-sm mb-1 text-muted">
					An issue is picked up at <b>Queued</b>, runs as one session, and ends
					either at <b>Needs a human</b>, <b>Awaiting release</b> or closed. The
					session is driven by the <code>issue-flow</code> skill, which this
					project gets from a plugin — see Plugins below.
				</p>
				{libraryHref && (
				...
				<div className="mt-6 border-t border-line pt-5">
					<Collapsible
						title={`Configured elsewhere — ${API_ONLY_KEYS.length} keys this screen doesn't edit`}
					>
