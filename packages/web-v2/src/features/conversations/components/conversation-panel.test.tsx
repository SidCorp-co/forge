...
describe("ConversationPanel \u00b7 what a slow or failing request must not do", () => {
  it("does not jump to the room a draft opened after the person had already moved on", async () => {
    ...
    // cm:guard the fourth is the outbox row's own id, which the send carries so `conversation.accepted`
    // comes back naming the row it belongs to; it is minted per row, so its shape is all this case can
    // say about it (ISS-1078).
    await waitFor(() =>
