...
export function OrgsTab() {
  ...
  if (orgsQ.isError) {
    return <ErrorState message={formatApiError(orgsQ.error)} onRetry={() => orgsQ.refetch()} />;
  }
      ...
      <Card>
        <CardContent>
          ...
          <p className="fg-body-sm mb-4 text-muted">
            Every project lives in exactly one org. Org owners/admins manage all of its projects;
            org members still need a per-project invite.
          </p>
      ...
      onSubmit={(e) => {
        ...
        create.mutate(
          { name: name.trim(), slug: slug.trim() },
          {
            ...
            onError: (err) => toast({ title: "Request failed", description: formatApiError(err), tone: "error" }),
          },
        );
      }}
    >
      ...
      <Button type="submit" disabled={!name.trim() || !slug.trim() || create.isPending}>
        Create org
      </Button>
    </form>
  );
