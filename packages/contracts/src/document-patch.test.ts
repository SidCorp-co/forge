...
describe('applyDocumentPatch', () => {
  it('leaves a key the patch does not name untouched at every depth', () => {
    const stored = {
      states: { open: { deviceIds: ['d1'], allowedTools: ['Read'] }, in_progress: { enabled: true } },
      legacyKeyNoSchemaSurfaces: { kept: 1 },
    };
    const next = applyDocumentPatch(stored, { states: { open: { deviceIds: ['d2'] } } });
    expect(next).toEqual({
      states: { open: { deviceIds: ['d2'], allowedTools: ['Read'] }, in_progress: { enabled: true } },
      legacyKeyNoSchemaSurfaces: { kept: 1 },
    });
  });
  ...
  it('replaces an array rather than merging it', () => {
    expect(applyDocumentPatch({ tools: ['a', 'b'] }, { tools: ['c'] })).toEqual({ tools: ['c'] });
  });

  it('creates a nested object the document does not have yet', () => {
    expect(applyDocumentPatch({}, { states: { open: { enabled: true } } })).toEqual({
...
describe('patchLeafPaths', () => {
  it('walks a nested object rather than claiming it', () => {
    expect(patchLeafPaths({ states: { open: { deviceIds: ['d1'] } }, enabled: true })).toEqual([
...
describe('comparePatchBase', () => {
  const stored = { states: { open: { deviceIds: ['d1'], allowedTools: ['Read'] } } };

  it('finds nothing where the caller read what is stored', () => {
    expect(
      comparePatchBase(stored, stored, { states: { open: { deviceIds: ['d2'] } } }),
    ).toEqual([]);
  });

  it('finds nothing where another path moved under the caller', () => {
    const moved = { states: { open: { deviceIds: ['d1'], allowedTools: ['Write'] } } };
    expect(comparePatchBase(moved, stored, { states: { open: { deviceIds: ['d2'] } } })).toEqual([]);
  });

  it('names the path, what was read and what is stored', () => {
    const moved = { states: { open: { deviceIds: ['dX'], allowedTools: ['Read'] } } };
    const conflicts = comparePatchBase(moved, stored, { states: { open: { deviceIds: ['d2'] } } });
  ...
  it('refuses a delete of a value that moved', () => {
    ...
    expect(comparePatchBase(moved, stored, { states: { open: { deviceIds: null } } })).toHaveLength(
      1,
    );
  });

  it('refuses a creation where somebody else got there first', () => {
    const conflicts = comparePatchBase({ intakeGate: { enabled: true } }, {}, {
      intakeGate: { enabled: false },
    });
    expect(conflicts).toEqual([{ path: 'intakeGate.enabled', base: undefined, stored: true }]);
  });

  it('reads an absent key and a stored null as the same value', () => {
    expect(sameStoredValue(null, undefined)).toBe(true);
    expect(comparePatchBase({ live: {} }, { live: { url: null } }, { live: { url: 'x' } })).toEqual(
      [],
    );
  });
});
...
describe('buildDocumentPatch', () => {
  ...
  it('sends a null for a key the caller dropped', () => {
    expect(buildDocumentPatch({ a: 1, b: 2 }, { a: 1 }).patch).toEqual({ b: null });
  });

  it('sends nothing where a key went from absent to null, which the store reads alike', () => {
    expect(buildDocumentPatch({ live: {} }, { live: { url: null } }).patch).toEqual({});
  });
