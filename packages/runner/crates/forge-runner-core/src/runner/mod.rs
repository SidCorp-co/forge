...
pub struct JobSpec {
    ...
    pub duplex: bool,
    // cm:guard the ONLY thing that keeps a duplex process off the box's session ceiling, and it is deliberately not derived from `issue_id`/`pat_token`/`step` — those differ between chat and a pipeline job by accident, this says it on purpose. Chat sets it false (owner decision 2026-09-04: a chat turn must never queue behind anything), pipeline duplex jobs set it true. A new caller that leaves it false spawns processes nothing bounds.
