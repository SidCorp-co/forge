...
mod tests {
    ...
    fn the_mcp_config_path_reaches_the_pane_quoted_and_without_strict() {
        let path = std::path::PathBuf::from("/home/o p/config/forge-runner/mcp/forge-master-mcp-x.json");
        let line = pane_argv(Some(&path))[2].clone();
        assert!(
            line.contains("--mcp-config '/home/o p/config/forge-runner/mcp/forge-master-mcp-x.json'"),
            "{line}"
        );
        assert!(
        ...
        let out = std::process::Command::new("sh")
            .arg("-c")
            .arg(format!("printf %s {}", shell_quote(&path.to_string_lossy())))
            .output()
            .expect("sh must run");
        assert_eq!(
            String::from_utf8_lossy(&out.stdout),
