...
- Backend is `packages/core` (Hono + Drizzle) per [RFC 0002](../../docs/rfcs/0002-replace-strapi-with-hono-drizzle.md). Strapi was removed at Phase 2.8-F1 — write all API code against `core`'s response shape (no `{ data, meta }` envelope).
