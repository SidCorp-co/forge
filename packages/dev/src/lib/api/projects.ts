...
// packages/core's updateProjectSchema is .strict() — only the fields below pass
...
export async function updateProject(documentId: string, data: Record<string, unknown>): Promise<Project> {
  ...
  if (dropped.length > 0) {
    console.warn(
      `[projects] updateProject: dropping unsupported field(s) ${dropped.join(", ")} — packages/core's PATCH /projects/:id only accepts ${[...PROJECT_PATCH_FIELDS].join(", ")} (TODO(iss-275)).`,
    );
  }
