...
-- to a new name rather than dropped. Running backwards is `rollback/0279_down.sql`,
...
BEGIN
  ...
  IF offenders IS NOT NULL THEN
    ...
    RAISE EXCEPTION
      E'ISS-1069: these projects hold a `preview_deploy` value the `environments` shape cannot represent, so this migration has written nothing:\n%\nCorrect each row named above and redeploy. Do NOT widen the check in 0279_environments.sql: a value quietly cleaned away here is a value the operator never learns they lost.',
      offenders;
  END IF;
