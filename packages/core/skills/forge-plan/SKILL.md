...
1. For each child workstream, create a child issue:
   ```
   forge_issues → create → { data: { title: "<child slice title>", description: "<scoped description>", status: "draft", priority: <inherit>, category: <inherit>, manualHold: false } }
   ```
   Children stay parked at `draft` until the parent is approved.
...
- Parent waiting → approved fires the cascade: every `draft`/`open` child flips to `approved` (manualHold cleared if set).
