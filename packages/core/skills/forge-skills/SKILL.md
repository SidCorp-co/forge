...
6. **Emit structured signals — don't bury machine-consumed data in the comment.** When a skill produces data an aggregator reads (friction, UX gaps, metrics, learnings), write it to its dedicated tool/table (`forge_feedback`, …) as a **non-blocking side-channel** — *in addition to* the human-facing comment and the status transition, never instead of them. The comment is for the reader; the structured row is for the machine. A failed emit must never change the skill's verdict.
7. **Consume a project-wide standard as a gated entry, not hardcoded prose.** A standard the pipeline must honor across issues (e.g. a UI completeness bar) lives as a knowledge entry and is referenced with an *"if the project has \<entry\>"* guard, so projects without it are unaffected. Reach for `injection: "always"` only when every step really does need the body in its preamble — ISS-1068 retired a subsystem that always-injected a 4 KB UI checklist into twelve projects and was cited back zero times in sixteen days. Read the standard from the entry; don't bake it into the skill body (that can't be tuned per project and drifts).
