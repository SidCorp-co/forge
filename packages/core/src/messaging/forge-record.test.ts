...
describe('the fence a body means as a record', () => {
  ...
  it('finds a record after an enclosing block closed on an indented fence', () => {
    const body = [
      `${FENCE}text`,
      'example',
      '  ```',
    ...
    expect(record?.kind).toBe('verdict');
    expect(record?.fields.map((f) => f.key)).toEqual(['criterion']);
  });

  it('reads a record written with carriage returns', () => {
    const body = [
      `${FENCE}forge-record`,
      'criterion: 13',
      'verdict: skipped',
  ...
  it('reads the on-fence shape written with carriage returns', () => {
  ...
  it('says where the block sits when the body carries carriage returns', () => {
    const body = ['Before.', '', `${FENCE}forge-record`, 'criterion: 13', FENCE, '', 'After.'].join(
      '\r\n',
    );
    const record = parseForgeRecord(body);
    expect(body.slice(0, record?.at)).toBe('Before.\r\n\r\n');
    expect(body.slice(record?.to)).toBe('\r\n\r\nAfter.');
  });
