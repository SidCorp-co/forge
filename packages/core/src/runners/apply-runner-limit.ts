...
// cm:edge naming -> packages/core/src/runners/quarantine.ts — both alarms key the wedge on the RUNNER id (`wedge:<runnerId>`), and `emitPipelineWedge` returns early while an unresolved notification carries that key, so whichever fault lands first is the only one the operator is told about until it clears. That is deliberate, and it is only sound because BOTH faults exclude the box from dispatch — an auth stamp by name, a quarantine by `quarantinedUntil` — so the second fault cannot accumulate while the first stands, and clearing the first resolves the wedge, which is what lets the next real fault alarm. Break either exclusion and this becomes a silenced alarm: `resolvePipelineWedge(runnerId)` must therefore be called from EVERY clear on this row.
