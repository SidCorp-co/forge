...
describe('EmbeddingsClient.embed', () => {
  ...
  it('classifies exhausted retries as unavailable, so callers can degrade', async () => {
    ...
    const client = new EmbeddingsClient(cfg, fetchFn as unknown as typeof fetch);
  ...
  it('leaves a non-retriable failure classified as it was', async () => {
    ...
    const client = new EmbeddingsClient(cfg, fetchFn);
    await expect(client.embed('hi')).rejects.not.toBeInstanceOf(EmbeddingUnavailableError);
  });
