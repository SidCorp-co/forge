...
const { assertDispatchable, gateReasonsForQueuedJobs } = await import('./queued-gates.js');
const { TRIGGER_STATUS_BY_JOB_TYPE, WORKING_STATUS_BY_JOB_TYPE } = await import(
  '../pipeline/registry.js'
);

// cm:edge contract -> packages/core/src/jobs/queued-gates.test.ts — the same fragment walker as the sibling suite; drizzle nests `sql` templates as queryChunks, so a shallow read of the template returns none of the gate text
...
/** The asserter's two round-trips: the jobs lookup, then the CASE. */
...
describe('L1b stale_trigger gate (ISS-789)', () => {
  async function reasonsSql(): Promise<string> {
  ...
  // cm:guard this ordering assertion is the whole safety property — read before issue_busy, the gate would call a job stale during the one window where a non-trigger status is legitimate (a sibling step mid-flight flipped it), and report the step queued behind the one doing the work as discardable
  it('is judged only after both issue_busy arms', async () => {
