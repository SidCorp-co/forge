...
// cm:guard pinned, not inherited from `RULES.patRead`/`RULES.patWrite`: those defaults are operator knobs (`RATE_LIMIT_PAT_READ_MAX`, `RATE_LIMIT_PAT_WRITE_MAX`) and the read half is budgeted for a whole box of sessions on one credential, which a job token is not. Lowering either must not throttle the fleet. Pinned here means 600 per class, reads and writes counted apart (ISS-961). The number is measured, not padded — over 30 days of `mcp_audit_log` a single project peaked at 108 calls in one minute (p50 2, p95 6, p99 10). A job mints its token once at dispatch and has no way to ask for another, so a 429 storm does not degrade the job, it stalls it. 600 is ~6x the observed peak and still a real ceiling on a credential that is bound to one project and dies with the job.
