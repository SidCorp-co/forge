...
describe('jobs/dispatcher', () => {
  ...
  // cm:guard the resume bound belongs to the RETRY path and is proved in resume-policy.test.ts. The block deleted from here exercised a FIRST dispatch resuming a prior session found by (issue, sessionGroup) — a lookup with no producer since ISS-897 removed `pipelineConfig.sessionGroups` — so every one of those cases passed vacuously the moment the offer stopped existing. Do not restore them without restoring the offer.
