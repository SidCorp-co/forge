...
// cm:guard this function is the only thing standing between a raised cap and a box whose runner cannot hold the repo-root lock. Every case below must keep resolving to 1 on the "not proven new enough" side; a test relaxed to accept the configured value there re-opens exactly the corruption the lock exists to prevent.
