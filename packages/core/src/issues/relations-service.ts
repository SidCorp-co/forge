...
import { z } from 'zod';
import { type IssueDependencyWriter, setIssueDependency } from './dependency-service.js';
...
export async function applyIssueRelations(
  ...
  for (const rel of relations ?? []) {
    ...
    const result = await setIssueDependency(
      {
        projectId,
        fromIssueId,
        toIssueId,
