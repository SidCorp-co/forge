/**
 * ISS-1160 — `GET /api/issues/:id/attachments` is one of the eight routes the
 * second judging pass found this defect spans. This file proves the WIRING: the
 * route parses `:id` as any non-empty string, reads `?projectId=`, hands both to
 * the shared `resolveIssueRouteRef`, and returns what it resolves — or surfaces
 * its refusal verbatim. The resolver's own resolution logic is
 * `issue-route-ref.test.ts`'s; `attachment-routes.test.ts` keeps its own uuid-path
 * authz coverage and does not grow for this — a separate file so neither pays
 * for the other's fixtures.
 */
...
vi.mock('../middleware/require-any-auth.js', () => ({
  requireAnyAuth: () => async (_c: unknown, next: () => Promise<void>) => next(),
}));
...
const { issueAttachmentRoutes } = await import('./attachment-routes.js');
...
describe('GET /api/issues/:id/attachments — forwards to the shared resolver (ISS-1160)', () => {
  ...
  it('a display key with ?projectId= reaches the resolver with both, unmangled', async () => {
    ...
    const res = await buildApp().request(
      `/api/issues/ISS-1185/attachments?projectId=${PROJECT_ID}`,
    );
    expect(res.status).toBe(200);
  ...
  it('surfaces the resolver refusal verbatim — a key with no project to scope it', async () => {
    ...
    const res = await buildApp().request('/api/issues/ISS-1185/attachments');
