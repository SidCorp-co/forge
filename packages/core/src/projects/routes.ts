...
projectRoutes.get("/", async (c) => {
	...
	return c.json(
		rows.map(({ memberRole, orgRole, apiKey, ...row }) => {
			const role = maxProjectRole(
				memberRole ?? null,
				orgDerivedProjectRole(orgRole ?? null),
			);
			return {
				...row,
...
projectRoutes.patch(
	...
	async (c) => {
		...
		if (patch.productionBranch !== undefined)
			updates.productionBranch = patch.productionBranch;
		if (patch.defaultDeviceId !== undefined)
			updates.defaultDeviceId = patch.defaultDeviceId;
		...
		if (patch.previewDeploy !== undefined)
			updates.previewDeploy = patch.previewDeploy;
		if (patch.webhookSecret !== undefined)
			updates.webhookSecret = patch.webhookSecret;
...
projectRoutes.get(
	...
	async (c) => {
		...
		return c.json({
			projectFacts:
				(ac.projectFacts as Record<string, string> | undefined) ?? {},
			projectFactsConfig:
				(ac.projectFactsConfig as
					| Record<string, { alwaysInject?: boolean }>
					| undefined) ?? {},
...
projectRoutes.patch(
	...
	async (c) => {
		...
		if (patch.projectFacts !== undefined) {
			...
			ac =
				merged === null
					? dropKey(ac, "projectFacts")
					: { ...ac, projectFacts: merged };
		}
		if (patch.projectFactsConfig !== undefined) {
			const merged = mergeProjectFactsConfig(
				ac.projectFactsConfig,
				patch.projectFactsConfig,
			);
			ac =
				merged === null
					? dropKey(ac, "projectFactsConfig")
					: { ...ac, projectFactsConfig: merged };
		}
...
const TEMPLATE_SETS: Record<
	string,
	ReadonlyArray<{ stage: string; skillName: string }>
> = {
	"forge-default": Object.entries(STATUS_TO_JOB_TYPE).flatMap(
		([status, mapping]) =>
...
projectRoutes.post(
	...
	async (c) => {
		...
		const [projectRow] = await db
			...
			.where(eq(projects.id, id))
			.limit(1);
		const currentAc = (projectRow?.agentConfig ?? {}) as Record<
			string,
			unknown
		>;
		const currentPipeline = (currentAc.pipelineConfig ?? {}) as Record<
			string,
			unknown
		>;
