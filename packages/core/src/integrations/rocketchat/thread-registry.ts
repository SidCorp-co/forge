...
// cm:guard the WHOLE triple and never the `tmid` alone, which is what this answered until ISS-1091:
// a follow-up round is posted into the room its thread lives in, and a caller that had only the
// `tmid` had to pair it with a room it resolved for itself — the project's, which since ISS-1091 may
// not be the room the first round went to. A `tmid` from one room sent to another posts a follow-up
// as a fresh message in a room nobody was answering in.
// cm:guard `rcq_threads_question_idx` is UNIQUE on `question_id`, so one decision has exactly one
// thread and this row is the destination of every round after the first. That is why a later round
// whose destination would differ is refused by name rather than opening a second thread.
...
// cm:guard it ANSWERS rather than returning void, and a caller that can take its thread id before it
// posts must take the anchor through this call rather than reading ownership and then posting: two
// questions anchored on one message both pass a read, both post, and this insert then drops one of
// them in silence — after which every reply in that thread resolves to one question and the other
// waits for ever. The take is the atomic step; the refusal is what the loser gets (ISS-1091
// criteria 20, 21).
