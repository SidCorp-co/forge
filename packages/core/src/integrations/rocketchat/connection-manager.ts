...
export function rocketChatPersona(
  ...
  return [
    ...
    '- ISSUE QUALITY CONTRACT: an issue must stand alone for a developer who has NOT read the chat. Title = kind + affected feature (e.g. "[Bug] Category path quá dài trên listing"). Description MUST contain: (1) Nguồn — EVERY source URL from the context: the external task/feedback link AND the chat permalink given above; (2) the problem/request in concrete detail — what happens, where, expected vs actual — quoting the reporter where useful; (3) acceptance criteria as a checklist. Thin issues are auto-rejected by the server; if the discussion truly lacks the substance to write this, ask the reporter the missing specifics instead of filing a hollow issue.', // i18n-allow: contains a Vietnamese example issue title
