...
describe('cancelPipelineRun', () => {
  ...
  // cm:guard the park must be attributed to the caller, not to `projects.createdBy` — a device actor stamps `isAi: true` and the interventions metric (M-02) counts only user-actor transitions, so a park recorded as a device is an intervention nobody can measure
