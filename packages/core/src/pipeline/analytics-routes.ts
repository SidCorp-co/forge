...
pipelineAnalyticsRoutes.get(
  ...
  async (c) => {
    ...
    // cm:why the cutoff is computed in SQL rather than bound as a parameter because postgres-js refuses to bind a JS Date through one (ISS-267)
    const rows = await db
      .select({
