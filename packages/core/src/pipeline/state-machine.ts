...
// cm:edge contract -> packages/core/src/pipeline/registry.ts#getPipelineRegistry — this table is now SERVED as the registry's `statusExits`, whole and in declared order, and the web-v2 status picker offers a rung exactly what its row holds. A row edited here changes a live menu, and a row REORDERED here moves which move reads as the forward one (ISS-982).
// cm:guard ADVISORY, NOT A GATE for what the server ACCEPTS — `canTransitionFree` below is still the only runtime check and it permits ANY non-draft from → ANY non-draft to. Reading a missing pair here as "illegal" has produced wrong conclusions and pointless multi-hop workarounds; what a missing pair now means is that the menu does not OFFER it, never that the server refuses it. The soft-skip resolver this guard used to name was deleted by ISS-897; nothing imports `canTransition` or `getAllowedTransitions`. It is kept because step 5 of the removal order makes it the gate — which is the one change that turns every row here from advice into a refusal, so a row that is merely stale becomes a rule.
