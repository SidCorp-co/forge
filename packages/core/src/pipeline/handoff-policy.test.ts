...
describe('resolveHandoffsPolicy', () => {
  ...
  it.each([
    ...
    'canonical inject list for step=%s = %j',
    (step, expected) => {
      expect(resolveHandoffsPolicy(undefined, step).injectFromSteps).toEqual(expected);
    },
  );

  it('non-handoff steps (clarify/release/custom/pm) default to empty inject', () => {
    expect(resolveHandoffsPolicy(undefined, 'clarify').injectFromSteps).toEqual([]);
  ...
  it('explicit enabled=false overrides default-on', () => {
    const r = resolveHandoffsPolicy(
      { handoffs: { enabled: false } } as never,
      'plan',
    );
  ...
  it('explicit injectFromSteps narrows non-handoff steps out (release / clarify ignored)', () => {
    const r = resolveHandoffsPolicy(
      {
        handoffs: {
          enabled: true,
    ...
    expect(r.injectFromSteps).toEqual(['triage', 'plan']);
  });
