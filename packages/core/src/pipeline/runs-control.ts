...
// cm:guard record the human who cancelled, never a synthesized device — a device actor writes `isAi: true` and attributes the park to `projects.createdBy`, which drops it out of the interventions metric (M-02) that counts user-actor transitions. The device fallback is for callers with no user in scope, not the normal path.
