...
export async function cancelPipelineRun(
  ...
  if (result.broadcast) {
    ...
    // cm:guard `fromStatus: 'running'` is FALSE when the run was `paused`, and on this path that is a new false statement rather than an inherited one — before this call an operator cancel emitted nothing, so `sentry-breadcrumbs.ts` now renders "running -> cancelled" for a run that was paused, on the surface the maintainer uses to reconstruct a run's history without the DB. Priced, not free: every one of the four subscribers keys on `toStatus`, and `emitCloseHook` in pipeline/runs.ts already records the same fixed value for the identical case. The exit condition is the honest value threaded through `applyKernelTransition`'s returning shape — NOT a pre-select here, which is a second round-trip and a second convention.
