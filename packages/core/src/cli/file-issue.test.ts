...
vi.mock('../issues/create-service.js', () => ({
  createIssue: (...args: unknown[]) => created(...(args as [])),
}));
vi.mock('../assistant/tools/issue-dedup.js', () => ({
  findDuplicateIssue: (...args: unknown[]) => duplicate(...(args as [])),
}));

const { CLI_DEDUP_OVERRIDE, CLI_DUPLICATE_CORPUS, CLI_DUPLICATE_THRESHOLD, fileIssueThroughCli } =
  await import('./file-issue.js');

const WRITER = {
  createdById: 'u1',
  createdVia: 'mcp' as const,
  actor: { agency: 'agent' } as never,
};
...
describe('a filing this door accepts', () => {
  ...
  it('forwards the relations the filing carried, which its own parts refusal tells a filer to send', async () => {
    reset();
    const relations = [
      { dependsOnId: '11111111-1111-4111-8111-111111111111', kind: 'blocks' as const },
    ];
