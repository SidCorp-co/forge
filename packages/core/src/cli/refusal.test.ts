...
import { duplicateGap, duplicateRefusal, noticeFor, SHAPE_HEAD, shapeRefusal } from './refusal.js';
...
describe('the refusal a filer meets', () => {
  ...
  it('opens by saying the filing was held rather than filed', () => {
    const read = readFiling({
      title: 'A title that reads fine',
      body: 'no headings here at all',
      category: 'feature',
    });
...
describe('the duplicate refusal', () => {
  ...
  it('names the flag that clears it, rather than a sentence nothing reads', () => {
    expect(duplicateGap(SEEN, OVERRIDE).clear).toContain(
      `send the filing again with \`${OVERRIDE}\``,
    );
  });
