...
// cm:guard fork on `drive`, and `jobType` is a sound proxy for the lane because `autonomousStepFor` is the ONLY producer of that type and the `stageEnum` on `POST /api/issues/:id/run-pipeline-step` excludes it — a drive job cannot be enqueued outside the autonomous lane. What reached a driver before this fork was not a stale tool name: it was a nine-rung ladder, `waiting`/`reopen`/`on_hold` (which `issues/autonomous-park.ts` rewrites on every write), `forge_step_start` and a full MCP catalogue, handed to a shell holding `$FORGE_PAT` and no MCP client.
