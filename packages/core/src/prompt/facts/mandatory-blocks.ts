...
// cm:guard the OTHER arm is not residue and must not be deleted as such: it is what `release_batch`, `smoke`, `reconcile` and `verify_skill` take — the four remaining entries of `RUNNER_CAPABILITIES` — and each of the four renders `PIPELINE_RULES` and `TOOL_REFERENCE` today. ISS-1047 read it as "the staged branch" from a comment that called it one, and the measurement said otherwise. The eight staged job types reach this function only through a `jobs` row a runner already refused `runner_unsupported_type`, so the arm's audience is exactly those four and not "everything that is not drive".
// cm:guard both arms must return the module constants BYTE-IDENTICALLY. These two blocks are the head of the shared prefix every job sends, so anything computed per-call here — a trim, a join, an interpolated project name — turns a broad Anthropic prompt-cache hit into a miss on every dispatch. `prompt/prompt-surface-identity.test.ts` pins both arms to their bytes.
