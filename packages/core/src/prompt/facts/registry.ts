...
- **A blocker releases its dependents by reaching \`developed\`, not by being stamped.** A \`blocks\` dependent is held out of the set a master reads until its blocker's status is one of \`developed\`, \`testing\`, \`awaiting_release\`, \`closed\`. A reopened blocker blocks again. \`merged_at\` gates nothing: a blocker whose code has landed but which sits at \`in_progress\` or \`on_hold\` still holds its dependents, so stamping is not how you unblock them — moving the blocker forward is. \`merged_at\` is still owed as the EVIDENCE that the work landed, which is what \`developed\` is earned on, and it auto-stamps only on leaving \`awaiting_release\`, the one base-merge state (there is no per-project \`mergeStates\` key — migration 0195 deleted it). So if you merge the issue branch to the base branch then PARK at that state (a manual gate the system won't auto-advance), nothing stamps it — and the stamp is the evidence your own \`developed\`/\`testing\` rungs are read against, so stamp it yourself right after the merge lands: \`forge_issues.mark_merged({ issueId, target: 'base' })\`. Forge never merges or stamps server-side; the \`## Merge required\` block carries the details. **Verify before you stamp.** \`merged_at\` is CALLER-ASSERTED — nothing server-side checks git. Confirm the commits are actually reachable from the target branch ON THE REMOTE (\`git fetch\`, then \`git merge-base --is-ancestor <sha> origin/<branch>\`; after a squash merge the sha never appears, so check the issue's diff is present instead) before you stamp or close. A push exit code, matching branch names, or "the previous step said so" is not evidence. Closing an abandoned issue whose code never landed does unblock its dependents, because \`closed\` is one of the statuses that releases them — and \`forge_issues.unmark\` withdraws the shipped-work claim but does NOT put them back, since nothing gates on the stamp. Move the issue back off \`closed\` to hold them again.
- **A blocker's \`merged_at\` is a claim, not proof.** Its status let you dispatch; the stamp beside it is caller-asserted and nothing verified it, and several projects have had a dependent build against code that was never on the base branch. Before you rely on a blocker's work, confirm it is actually there. If it is not: say so in a comment and set \`waiting\` (or \`reopen\` if it is your own issue's code) — do NOT silently build against it, and do NOT merge the blocker yourself.
...
- **forge_config** — read/write per-project settings: baseBranch, repoPath, releaseModel, liveBranch (read only under \`promote\`), categories, pipelineConfig, plugins. It carries NO project prose: \`projectFacts\` and \`projectFactsConfig\` were retired in ISS-1048 and a call naming either is refused by name. Prose is \`forge_knowledge\`, whose \`injection\` field (\`always\` | \`on_demand\` | \`none\`) is what the always-inject flag became.
