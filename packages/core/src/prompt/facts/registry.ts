...
- **Where to move next.** The \`## This State\` section below names the exact status to set on success and on a block — follow it. Otherwise follow the \`### Status ladder\` section — it is project-resolved and OVERRIDES the default. Only when neither is present, default forward along \`open → in_progress → developed → testing → awaiting_release → closed\`. That is the whole forward chain: one dispatch door, one worker, a review rung, a QA rung, one gate a person releases, one end. **\`confirmed\`, \`clarified\`, \`approved\`, \`waiting\` and \`tested\` are retired and must never be written.** They are still in the enum for the rows that already hold them, so \`forge_issues.update\` ACCEPTS them and says nothing — and nothing dispatches at any of them, so an issue you put there is stranded until a person moves it by hand. An \`autonomous\` project has no ladder to walk at all: its driver writes \`open\`, \`in_progress\`, \`needs_info\`, \`closed\`, \`dropped\` and nothing else, so reach for the status your step names, never the next rung.
...
// cm:guard this array is the full forward sequence and PIPELINE_RULES embeds the same chain in prose — change one and the prompt states two ladders with nothing comparing them. It is SIX rungs since 2026-09-10 (docs/flows/issue-status-lifecycle.html) — `developed` and `testing` are the review and QA gates, kept because they are where the proof is owed. The four it lost are why the rest went: while it named `confirmed` `clarified` `approved` `developed` `testing` `tested`, agents walked them — 153 hops across 4 projects in the 3 hours before it was cut — and each one lands on a status no job dispatches at, so a session that dies mid-ladder strands the issue with nothing in core to re-enter it. 45 rows were sitting like that when this was measured.
...
- \`confirmed\`, \`clarified\`, \`approved\`, \`waiting\`, \`tested\` are retired but still IN the enum, so the write SUCCEEDS and nothing warns you. Nothing dispatches at any of them, so the issue is stranded there until a person moves it by hand. Never write one, whatever a skill's exit table says.`;
    },
  },
  {
