...
// cm:guard this array is the full forward sequence and PIPELINE_RULES embeds the same chain in prose — change one and the prompt states two ladders with nothing comparing them. It is FOUR rungs since 2026-09-10 (docs/flows/issue-status-lifecycle.html), and the six it lost are why: while it named `confirmed` `clarified` `approved` `developed` `testing` `tested`, agents walked them — 153 hops across 4 projects in the 3 hours before it was cut — and each one lands on a status no job dispatches at, so a session that dies mid-ladder strands the issue with nothing in core to re-enter it. 45 rows were sitting like that when this was measured.
...
- \`confirmed\`, \`clarified\`, \`approved\`, \`developed\`, \`testing\`, \`tested\` are retired but still IN the enum, so the write SUCCEEDS and nothing warns you. Nothing dispatches at any of them, so the issue is stranded there until a person moves it by hand. Never write one, whatever a skill's exit table says.`;
    },
  },
  {
