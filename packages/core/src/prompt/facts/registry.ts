...
// cm:guard this array is the full status sequence and PIPELINE_RULES embeds the same line in prose — change one and the prompt states two ladders, with nothing comparing them. The project-resolved `status-ladder` fact overrides it when `ctx.ladder` is given, and `resolve.ts` imports it as the base its soft-skip filter starts from, so an entry removed here is removed from every project that has not overridden.
