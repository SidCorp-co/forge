...
- **Decompose is system-owned — do NOT hand-set parent/child statuses.** When you decompose a parent into children, core parks the parent at \`waiting\` (the review gate) and creates the children at \`draft\`. A human approving the parent auto-cascades the children to whichever status that project's driver dispatches from — \`approved\` on a staged project, \`open\` on an \`autonomous\` one. The parent's own forward work is held by the dispatcher until ALL children's code is merged (each child's \`merged_at\` set, or a closed child on a structurally unstampable base), then the parent runs its integration LAST. The kickoff is anchored to these system transitions — manually moving a decompose parent or child breaks it.
...
4. A human approves the parent, and the cascade promotes every child to whichever status that project's driver dispatches from: on a staged project the parent goes \`waiting → approved\` and the children land at \`approved\`; on an \`autonomous\` project it goes \`waiting → open\` and they land at \`open\`, because \`approved\` is a status that driver never reads.
5. The dispatcher holds the parent's own forward work until ALL children have \`merged_at\` set (or are closed on a structurally unstampable base) — i.e. each child's code is on the base branch; the parent then runs its integration LAST. A child unblocks the parent (and any \`blocks\` sibling) at the base-merge moment via \`merged_at\` — it does NOT need to reach \`released\`/\`closed\`. If a child merges its branch then parks at a manual gate, it MUST stamp its own \`merged_at\` (\`forge_issues.mark_merged target=base\`, see Pipeline Rules) or the parent and downstream siblings stall silently.
