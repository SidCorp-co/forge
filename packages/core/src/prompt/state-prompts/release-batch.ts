...
Do NOT call \`forge_step_start\`. Read the batch FIRST:
\`forge-runner api projects/{projectId}/release-batches/{runId}\`.

### Ordering contract (load-bearing — follow exactly)
1. \`GET projects/{projectId}/release-batches/{runId}\` → roster, releaseNotes per issue, branches, deployPlanned.
...
3. \`POST projects/{projectId}/release-batches/{runId}/finish\` with \`{ commit }\` → every claimed
   issue closes. Report closed/failed. \`commit\` is the SHA you pushed to the production branch.
...
with 409 RELEASE_NOT_VERIFIED unless the live build both changed and matches your \`commit\`. You
...
→ \`POST projects/{projectId}/release-batches/{runId}/abort\` with \`{ reason }\` — claims released, NOTHING closed.
