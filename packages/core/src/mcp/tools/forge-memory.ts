...
export const forgeMemorySearchTool: ContextScopedMcpToolFactory = ({ principal }) => ({
  ...
  description:
    'Search project memory (issues, comments, jobs, notes, knowledge, decisions, policies). strategy: "semantic" (default, cosine-similarity scores), "keyword" (Postgres FTS — exact identifiers, error codes), or "hybrid" (RRF fusion of both; scores are fused ranks, not similarity). On a project whose admin turned on rerank, a hybrid result may come back `reranked: true` — read hits in list order (`rerankPosition`), not by `score`. On a project with relation expansion on, rows carrying `via` are one-hop dependency neighbours of an issue hit, appended after the ranked hits with score 0 — context, not matches. A hit carrying `stale: true` was superseded (`supersededBy` names what by); those are sorted BELOW every fresh hit and `demotedStale` counts how many moved — they are demoted rather than dropped because a stale row may be the only one that answers, so read one only after the fresh ones and check what superseded it. Hits are point-in-time: verify against live code before acting, then report the outcome via `forge_memory.feedback` (confirmed|outdated) — that write-back is how stale memory gets cleaned instead of waiting on slow usage decay. Step handoffs live in their own table — use `forge_step_handoff.get` for those. Requires the caller to be a member of the given projectId.',
