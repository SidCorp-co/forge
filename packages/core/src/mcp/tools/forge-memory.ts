...
export const forgeMemorySearchTool: ContextScopedMcpToolFactory = ({ principal }) => ({
  ...
  description:
    'Search project memory (issues, comments, jobs, notes, knowledge, decisions, policies). A semantic or hybrid search costs one embedding call to the embeddings service (`embedMs` in the result names what it took, `took_ms` the whole); a keyword search costs none. When the question names an issue key, a status or a count, answer from the tracker (`forge issue …`) and do not search. strategy: "semantic" (default, cosine scores), "keyword" (Postgres FTS — exact identifiers, error codes), "hybrid" (RRF fusion of both; scores are fused ranks). With rerank on, a hybrid result may come back `reranked: true` — read hits in list order. With relation expansion on, rows carrying `via` are one-hop neighbours of an issue hit, appended after the ranked hits with score 0 — context, not matches. A hit carrying `stale: true` was superseded (`supersededBy` names what by).',
