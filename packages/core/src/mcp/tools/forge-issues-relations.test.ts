...
// cm:guard `applyIssueRelations` calls these transaction-scoped exports across a module boundary, which is the only reason overriding them works — move the implementation back inside forge-issues.ts and the relation tests start hitting the real DB chain and pass for the wrong reason
...
const { forgeIssuesTool, serialize } = await import('./forge-issues.js');
...
beforeEach(() => {
  vi.clearAllMocks();
  mutateDependencyMock.mockResolvedValue({
    id: 'dep-id-1',
    created: true,
    updated: false,
    effect: null,
...
it('serializes the recorded integration branch for parent agents', () => {
  expect(
    serialize({
      ...baseIssueRow,
      metadata: { integrationBranch: 'iss-868-relations' },
    }),
  ).toMatchObject({ metadata: { integrationBranch: 'iss-868-relations' } });
});
...
it('update passes validUntil through so an existing edge can be retracted', async () => {
  stageUpdate();
  mutateDependencyMock.mockResolvedValueOnce({
    id: 'dep-id-1',
    created: false,
    updated: true,
  } as never);
...
it('update commits the edge BEFORE the status transition that wakes the dispatcher', async () => {
  ...
  const edgeAt = mutateDependencyMock.mock.invocationCallOrder[0];
...
it('attributes the edge to the PAT user, not to the synthetic device standing in for it', async () => {
  ...
  expect(finalizeDependencyMutationMock).toHaveBeenCalledWith(
    expect.anything(),
    expect.anything(),
