 ...
 * convention.
 *
 * All five tools require the calling device's owner to be a member of the
 * run's project.
 */
...
async function loadRunForDevice(
  device: Parameters<DeviceScopedMcpToolFactory>[0],
  runId: string,
) {
  const [row] = await db
    .select()
...
export const forgePipelineRunsGetTool: DeviceScopedMcpToolFactory = (device) => ({
  name: 'forge_pipeline_runs.get',
  description:
    'Fetch a single pipeline run plus a per-status job count breakdown. Requires device owner to be a member of the run’s project.',
  inputSchema: zodToMcpSchema(runIdInputSchema),
  handler: async (args) => {
    const { runId } = runIdInputSchema.parse(args);
    const run = await loadRunForDevice(device, runId);
...
export const forgePipelineRunsPauseTool: DeviceScopedMcpToolFactory = (device) => ({
  name: 'forge_pipeline_runs.pause',
  description:
    'Pause a running pipeline run. Idempotent on paused runs. Throws CONFLICT on terminal runs (completed/failed/cancelled). Requires project membership.',
  inputSchema: zodToMcpSchema(runIdInputSchema),
  handler: async (args) => {
    const { runId } = runIdInputSchema.parse(args);
    await loadRunForDevice(device, runId);
    const run = await pausePipelineRun(runId);
    return { run };
  },
});

export const forgePipelineRunsResumeTool: DeviceScopedMcpToolFactory = (device) => ({
  name: 'forge_pipeline_runs.resume',
  description:
    'Resume a paused pipeline run. Idempotent on already-running runs. Throws CONFLICT on terminal runs. Requires project membership.',
  inputSchema: zodToMcpSchema(runIdInputSchema),
  handler: async (args) => {
    const { runId } = runIdInputSchema.parse(args);
    await loadRunForDevice(device, runId);
...
export const forgePipelineRunsCancelTool: DeviceScopedMcpToolFactory = (device) => ({
  name: 'forge_pipeline_runs.cancel',
  description:
    'Cancel a pipeline run. Marks the run cancelled, cancels queued+dispatched jobs of the run, transitions linked agent_sessions to failed with reason=pipeline_cancelled, and broadcasts agent:abort to each affected device. Idempotent on already-cancelled runs. Throws CONFLICT on completed/failed.',
  inputSchema: zodToMcpSchema(runIdInputSchema),
  handler: async (args) => {
    const { runId } = runIdInputSchema.parse(args);
    await loadRunForDevice(device, runId);
