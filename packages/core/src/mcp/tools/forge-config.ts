...
function formatBaseResponse(
	row: Awaited<ReturnType<typeof readProjectConfig>>,
) {
	...
	return {
		...
		config: {
			...
			pipelineConfig:
				(ac.pipelineConfig as Record<string, unknown> | undefined) ?? null,
			stateContext:
				(ac.stateContext as Record<string, unknown> | undefined) ?? null,
			projectFacts:
	...
	handler: async (args) => {
		...
		if (input.action === "update") {
			...
			if (input.stateContext !== undefined) {
				...
				const mergedSc = mergeStateContext(
					currentAc.stateContext,
					input.stateContext,
				);
			...
			if (input.projectFacts !== undefined) {
				...
				if (!row) throw new Error("NOT_FOUND: project not found");
				const currentAc = (row.agentConfig ?? {}) as Record<string, unknown>;
				const mergedFacts = mergeProjectFacts(
					currentAc.projectFacts,
					input.projectFacts,
				);
				const nextAc: Record<string, unknown> =
					mergedFacts === null
						? Object.fromEntries(
								Object.entries(currentAc).filter(([k]) => k !== "projectFacts"),
							)
			...
			if (input.projectFactsConfig !== undefined) {
				...
				const nextAc: Record<string, unknown> =
					mergedConfig === null
						? Object.fromEntries(
								Object.entries(currentAc).filter(
									([k]) => k !== "projectFactsConfig",
								),
							)
