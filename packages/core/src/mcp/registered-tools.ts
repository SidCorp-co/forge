...
// cm:guard when you do read `mcp_audit_log`, split it by `device_id IS NOT NULL` / `token_id IS NOT NULL` and by NOTHING else — `user_id` is populated for a device caller too (it is stamped `device.ownerId`), so a split on it reads 100% user and 0 device for every tool in the table. Measured 2026-09-01: `has_user_id` equals `device + pat` exactly on all six tools `7f0c5a56` deleted after claiming an audit-log split had cleared them, and the fleet hit one of them at 09:07 that same day and read `not_found`. Count over the WHOLE table and normalise the spelling — `count(*) ... WHERE replace(tool,'.','_') = <name with dots replaced>`, no date filter — because this column stores `request.params.name` verbatim and agents do send the underscore form the MCP client shows them: `forge_step_handoff_write` has 21 rows across seven boxes, `forge_skills_list` 32, `forge_memory_search` 15, every one `not_found`, and no query for the dotted name finds any of them. FULL OUTER join the registry against the aggregate, and read BOTH directions as findings: a tool nothing has EVER called has no row in the log at all, so an inner join drops exactly the tools this rule is looking for — that is why the wave-3 pass reported one device-free tool and `forge_memory.revisions`, at zero rows lifetime, was not it — and a name that was CALLED but is not registered has no registry row, so a plain LEFT from the registry drops the misspelling evidence that is the other half of this (`forge_memory_search`, 15 rows, every one `not_found`). "Lifetime" holds because this table has NO retention window — `pipeline/retention/policy.ts` states that and why, ISS-1027 deleted the unwired 90-day `enforceMcpAuditRetention` that contradicted it, and `GET /api/admin/mcp-audit/tools` returns both `retention` and `oldestRow` so you can see the rule and the evidence rather than trusting a doc. That route (ISS-946) is where all of this is already implemented correctly, so read a count off it instead of writing the query again; it is admin-only ON PURPOSE, because these counts are cross-project, and an agent gathers the refusals while a human supplies the clearance.
