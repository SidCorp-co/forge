...
- Project settings/secrets live in Forge, not the repo: test creds & preview URLs via \`forge_projects.get\` → \`previewDeploy\`; pipeline/process config via \`forge_config\` (never returns creds/URLs); project prose (build commands, rules, guides) via \`forge_knowledge\`, never \`forge_config\`.
