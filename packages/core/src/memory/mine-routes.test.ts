...
vi.mock('./mine-service.js', () => ({
  ...
  findMine: (...a: unknown[]) => findMine(...(a as [])),
}));
const effectiveProjectRole = vi.fn(
  async (_userId: string, _projectId: string): Promise<{ role: string } | null> => ({
    role: 'member',
  }),
);
const assertProjectAccess = vi.fn(async () => undefined);
vi.mock('../lib/authz.js', () => ({
  effectiveProjectRole: (...a: unknown[]) => effectiveProjectRole(...(a as [string, string])),
  assertProjectAccess: (...a: unknown[]) => assertProjectAccess(...(a as [])),
}));
...
describe('GET /api/memory/mine', () => {
  ...
  it('lists the caller’s own notes, by the caller’s id', async () => {
    ...
    expect(await res.json()).toEqual({
      items: [{ id: NOTE_ID, projectId: 'p1', textContent: 'deploys on Thursdays' }],
    });
  ...
  it('leaves out a note in a project the caller’s credential may not read', async () => {
    listMine.mockResolvedValue([
      ...
      { id: '44444444-4444-4444-8444-444444444444', projectId: 'p2', textContent: 'mine, fenced' },
    ]);
    effectiveProjectRole.mockImplementation(async (_u: string, projectId: string) =>
      projectId === 'p1' ? { role: 'member' } : null,
    );
    const res = await authed('/api/memory/mine');
    expect(((await res.json()) as { items: Array<{ id: string }> }).items.map((i) => i.id)).toEqual(
      [NOTE_ID],
    );
  });
