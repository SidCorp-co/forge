...
export function renderTerminationBlock(opts: {
  ...
  return [
    ...
    '',
    '1. Call `forge_step_handoff.write` with EXACTLY these scope fields (do not change them):',
    '',
    '   ```json',
    '   {',
    ...
    '   error, fix the `payload` and retry (max 3 attempts).',
    '',
    '3. ONLY AFTER `forge_step_handoff.write` returns success, respond with exactly',
    '       DONE',
