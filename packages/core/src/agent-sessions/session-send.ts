...
// cm:guard `inject` and `answer` write an audited `job_events` row in the SAME call, because both silently change what a running agent does and the interventions-per-issue metric (VISION §1 ②) counts exactly that. An unaudited `inject` is a hole in the metric, not an omission the RFC stage may defer.
