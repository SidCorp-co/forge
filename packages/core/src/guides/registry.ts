...
The runtime gate is permissive: **any status may move to any status, except that nothing may move INTO \`draft\`**, and \`draft\` itself may only leave to \`open\`, \`developed\`, \`closed\` or \`dropped\`. That is the whole rule. \`dropped\` is legal but **terminal with no exit** — unlike \`closed\`, which can be reopened, a dropped issue can only be re-filed; the recommended discard for non-work is still \`closed\` + \`unmark\`, per the table above.
