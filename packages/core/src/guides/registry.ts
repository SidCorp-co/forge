...
**\`closed\` means the work shipped, and \`dropped\` is the exit for everything else.** A close is refused (\`CLOSE_REQUIRES_SHIPPED\`) while the issue carries no \`merged_at\`, and the database refuses the same write whatever route it took — there is no second column to consult and no way to reach \`closed\` without having shipped. Use \`dropped\` for anything discarded: a note, a question, a duplicate, something already done. Where the work DID land outside the pipeline, claim it first with \`forge_issues\` \`mark_merged\` naming where it landed, then close.
