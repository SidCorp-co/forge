...
// cm:guard `total` is the COUNT and `shown` is capped: a panel rendering `shown.length` as the figure is the truncation-as-truth defect ISS-988 criterion 43 exists to refuse.
...
export interface PulseLiveness {
  ...
  runners: { online: number; draining: number; total: number };
}
