...
async function selectAbandoned(
  ...
  const rows = (await db.execute(sql`
    SELECT i.id, i.iss_seq, i.title, i.status, i.project_id, p.slug AS project_slug,
           coalesce(
             (SELECT max(coalesce(j.finished_at, j.acked_at, j.dispatched_at, j.queued_at))
              ...
              WHERE j.issue_id = i.id OR r.issue_id = i.id),
             i.updated_at
           ) AS idle_since
    FROM issues i
    ...
    WHERE i.project_id IN (${idList(projectIds)})
      ...
      AND NOT EXISTS (
        SELECT 1 FROM jobs j LEFT JOIN pipeline_runs r ON r.id = j.pipeline_run_id
        WHERE (j.issue_id = i.id OR r.issue_id = i.id) AND j.status IN (${idList(LIVE)})
      )
