...
// cm:guard FOUR middlewares verify a device token, and NONE of them hands the device its owner's account authority. Until ISS-927 this one did — `c.set('userId', device.ownerId)` — and it was the single place in the codebase where a credential silently became a person. `requireAuth` rejects devices outright and `requireUserOrDevice`, `requireDevice` and `requirePatOrDevice` (`/mcp`) all make the device its own principal with `userId` left unset so `loadProjectAccess` fails closed. There is no longer a middleware to pick when you "mean" ambient owner authority, because the answer is now to give the caller a token: a job gets `job:<id>` at claim, an unattended session gets `session:<id>` at `agent:start`. If a new caller class appears with neither, mint it one — do not restore this branch.
