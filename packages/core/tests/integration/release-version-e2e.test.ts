...
describe('the database is what refuses a version that is not three integers', () => {
  it.each([
    ...
    ['latest'],
    [''],
    // A tenth digit. It parses as three integers and would pass a CHECK that only counted dots,
    // and then `string_to_array(...)::int[]` throws on EVERY later read for this project — the
    // cast failing at read time for a value accepted at write time. Nine digits is the bound.
    ['2147483648.1.0'],
    ['0.1000000000.0'],
  ])('refuses %j by the name of the CHECK', async (bad) => {
...
describe('a re-cut is refused unless the release it names actually failed', () => {
  it('refuses a release that is still running, naming its status', async () => {
    ...
    await expect(cutRelease(() => projectId, open.version)).rejects.toThrow(/is still running/);
  });
  ...
  it('refuses a minor that would leave the nine-digit domain, by its own name', async () => {
    // The CHECK would refuse this write too, but it would name itself and leave the caller to work
    // out which rule they hit. Plant the last storable minor and ask for the next one.
    const last = await cutRelease();
    await db.execute(
      sql`UPDATE pipeline_runs SET release_version = '0.999999999.0' WHERE id = ${last.runId}`,
    );
    await setRunStatus(last.runId, 'cancelled');

    await expect(cutRelease()).rejects.toThrow(/RELEASE_VERSION_EXHAUSTED/);
    await expect(cutRelease()).rejects.toThrow(/0[.]1000000000[.]0/);
  });

  it('refuses a re-cut whose patch would leave that domain, by the same name', async () => {
    const last = await cutRelease();
    await db.execute(
      sql`UPDATE pipeline_runs SET release_version = '0.4.999999999' WHERE id = ${last.runId}`,
    );
    await setRunStatus(last.runId, 'cancelled');

    await expect(cutRelease(() => projectId, '0.4.999999999')).rejects.toThrow(
      /RELEASE_VERSION_EXHAUSTED/,
    );
  });

  it('refuses an empty `recutOf` rather than quietly cutting a fresh minor', async () => {
    const failed = await cutRelease();
    await setRunStatus(failed.runId, 'cancelled');

    // A caller who sent the field asked for a re-cut. Read '' as an omission and this returns
    // 0.2.0 — a new release wearing a number nobody asked for, and the burn rule's one blind spot.
    await expect(cutRelease(() => projectId, '')).rejects.toThrow(ReleaseRecutRefusedError);
    await expect(cutRelease(() => projectId, '')).rejects.toThrow(/it is not a version/);
  });
