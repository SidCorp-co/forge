...
// ISS-34: accept either the raw AgentStatus from task types or the derived
// display status (which adds 'stalled'). Falsy / 'idle' renders nothing —
// callers passing a fully-resolved session state get the right glyph.
...
export function AgentStatusIndicator({ status }: { status: IndicatorStatus }) {
  ...
  if (status === 'queued') {
    // Violet clock + slow pulse → reads as "waiting in line", visually
    // distinct from the running blue dot so users can tell at a glance.
    return (
      <svg
        className="h-3.5 w-3.5 animate-pulse text-violet-500 dark:text-violet-400"
        viewBox="0 0 20 20"
        fill="currentColor"
        aria-label="queued"
      >
  ...
  if (status === 'stalled') {
    // Amber warning triangle — the worker may still be alive but hasn't
    // emitted a heartbeat within STALLED_THRESHOLD_MS. The sweeper will
    // promote this to failed once it crosses HEARTBEAT_TIMEOUT_MS.
    return (
      <svg
